...
def test_tutorial_simulation():
    ...
    oracle.fit_GRN_for_simulation(
        alpha=10, use_cluster_specific_TFdict=True, verbose_level=1
    )
    ...
    oracle.estimate_transition_prob(
        n_neighbors=200, knn_random=True, sampled_fraction=1
    )
    # Calculate embedding
    ...
    oracle.plot_simulation_flow_on_grid(
        scale=scale_simulation, ax=ax, show_background=False
    )
    ...
    gradient.transfer_data_into_grid(
        args={"method": "polynomial", "n_poly": 3}, plot=True
    )
    # Calculate graddient
    ...
    dev.plot_simulation_flow_on_grid(
        scale=scale_simulation, show_background=False, ax=ax
    )
