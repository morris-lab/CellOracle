...
class Pseudotime_calculator:
    ...
    def __init__(
        ...
        if adata is not None:
            if (obsm_key is not None) & (cluster_column_name is not None):
                ...
                print(
                    "Please set obsm_key and cluster_column_name to instantiate Pseudotime_calculator with anndata."
                )
    ...
    def set_lineage(self, lineage_dictionary):
        ...
        for lineage, clusters in lineage_dictionary.items():
            self.adata.obs[lineage] = self.adata.obs[self.cluster_column_name].isin(
                clusters
            )
            self.adata.obs[lineage] = (
                self.adata.obs[lineage].astype("str").astype("category")
            )
    ...
    def set_root_cells(self, root_cells):
        ...
        for lineage, root_cell in root_cells.items():
            if lineage not in self.adata.obs.columns:
                print(
                    f"The key, {lineage} is not in the lineage data. Make sure the root_cells has correct format."
                )
            if root_cell not in self.adata.obs.index[self.adata.obs[lineage] == "True"]:
                print(
                    f"The cell, {root_cell} in the {lineage} was not found in your adata. Please make sure the format is correct."
                )
    ...
    def get_pseudotime_per_each_lineage(self, root_cells=None, plot=False):
        ...
        pseudotime_estimation_for_each_lineage(
            adata=self.adata, root_cells=self.root_cells
        )
        ...
        if main_pseudotime.isna().sum() > 0:
            if main_pseudotime.isna().mean() >= 0.5:
                raise ValueError(
                    "Found too many cells that failed to get pseudotime. Please check lineage dictionary."
                )
            else:
                # Replace na and inf with NN's value
                y_filled = _fill_inf_and_na(
                    X=self.adata.obsm[self.obsm_key],
    ...
    def plot_cluster(self, s=10, fontsize=8):
        ...
        color_dict = _adata_to_color_dict(
            adata=self.adata, cluster_use=self.cluster_column_name
        )
        ...
        for cluster, color in color_dict.items():
            ...
            plt.scatter(
                embedding[idx, 0], embedding[idx, 1], color=color, s=s, label=cluster
            )
    ...
    def plot_lineages(self, s=10):
        ...
        for lineage in self.lineage_dictionary.keys():
            color = [
                "#EC7063" if i == "True" else "#D0D3D4"
                for i in self.adata.obs[lineage].values
            ]
    ...
    def plot_root_cells(self, s=10):
        ...
        for lineage, root_cell in self.root_cells.items():
            ...
            plt.scatter(embedding[:, 0], embedding[:, 1], color=color, s=s)
            plt.scatter(
                embedding[root_cell_idx, 0],
                embedding[root_cell_idx, 1],
                color="black",
                s=s * 10,
                label="root_cell",
            )
    ...
    def plot_pseudotime(self, s=10, cmap="rainbow"):
        ...
        pseudotime_list = [f"Pseudotime_{i}" for i in self.root_cells.keys()] + [
            "Pseudotime"
        ]
        for pseudotime in pseudotime_list:
            ...
            fig = plt.figure()
            plt.scatter(
                embedding[idx, 0], embedding[idx, 1], c=color[idx], s=s, cmap=cmap
            )
...
def pseudotime_estimation_for_each_lineage(adata, root_cells):
    ...
    for lineage, root_cell in root_cells.items():
        ...
        adata.obs.loc[adata_.obs.index.values, f"Pseudotime_{lineage}"] = adata_.obs[
            "dpt_pseudotime"
        ]
