...
class Oracle_development_module(Data_strage):
    ...
    def get_hdf5_info(self):
        ...
        # Misc list
        dic["misc_list"] = np.unique(
            [i.split("/")[1] for i in self._names if len(i.split("/")) >= 2]
        )
        #
        unique_keys = np.unique(
            ["/".join(i.split("/")[:2]) for i in self._names if len(i.split("/")) >= 2]
        )
        ...
        for i in dic["misc_list"]:
            misc_gene_dictionary[i] = [
                gene for gene, misc in dic["gene_misc_lists"] if misc == i
            ]
    ...
    def load_differentiation_reference_data(self, gradient_object):
        ...
        self.pseudotime_on_grid = (
            gradient_object.pseudotime_on_grid.copy()
        )  # shape = (n_grid*n_grid, )
        self.mass_filter_reference = (
            gradient_object.mass_filter.copy()
        )  # shape = (n_grid*n_grid, )
        self.mass_filter_whole_reference = (
            gradient_object.mass_filter_whole.copy()
        )  # shape = (n_grid*n_grid, )
        self.gridpoints_coordinates = (
            gradient_object.gridpoints_coordinates.copy()
    ...
    def load_perturb_simulation_data(
        self,
        oracle_object,
        cell_idx_use=None,
        name=None,
        ...
        if cell_idx_use is None:
            ...
            oracle_object_ = subset_oracle_for_development_analysiis(
                oracle_object=oracle_object, cell_idx_use=cell_idx_use
            )
        ...
        oracle_object_.calculate_grid_arrows(
            smooth=smooth,
            steps=(self.n_grid, self.n_grid),
    ...
    def calculate_inner_product(self):
        ...
        self.inner_product_random = np.array(
            [np.dot(i, j) for i, j in zip(self.flow_rndm, self.ref_flow)]
        )

    def calculate_digitized_ip(self, n_bins=10):
        inner_product_df = pd.DataFrame(
            {
                "score": self.inner_product[~self.mass_filter_simulation],
                "score_randomized": self.inner_product_random[
                    ~self.mass_filter_simulation
                ],
        ...
        inner_product_df["pseudotime_id"] = np.digitize(
            inner_product_df.pseudotime, bins
        )
    ...
    def plot_pseudotime(
        ...
        plot_pseudotime(
            self=self, ax=ax, s=s, show_background=show_background, cmap=cmap, args=args
        )
    ...
    def plot_selected_pseudotime_on_grid(
        ...
        plot_selected_pseudotime_on_grid(
            self=self,
            ax=ax,
            pseudotime_selected=pseudotime_selected,
            s=s,
            show_background=show_background,
            args=args,
        )
    ...
    def plot_inner_product_on_pseudotime(
        ...
        plot_inner_product_on_pseudotime(
            self=self, ax=ax, vm=vm, s=s, vmin=vmin, vmax=vmax, cmap=cmap, args=args
        )

    def plot_inner_product_as_box(self, ax=None, vm=1, vmin=None, vmax=None, args={}):
        plot_inner_product_as_box(
            self=self, ax=ax, vm=vm, vmin=vmin, vmax=vmax, args=args
        )
    ...
    def visualize_development_module_layout_2(
        ...
        fig = visualize_development_module_layout_2(
            self=self,
    ...
    def visualize_development_module_layout_1(
        ...
        fig = visualize_development_module_layout_1(
            self=self,
            scale_for_pseudotime=scale_for_pseudotime,
            scale_for_simulation=scale_for_simulation,
            s=s,
    ...
    def visualize_development_module_layout_0(
        ...
        fig = visualize_development_module_layout_0(
            self=self,
            scale_for_pseudotime=scale_for_pseudotime,
            scale_for_simulation=scale_for_simulation,
            s=s,
