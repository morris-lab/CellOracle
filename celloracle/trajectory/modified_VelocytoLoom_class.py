...
class modified_VelocytoLoom:
    ...
    def plot_pca(
        self, dim: List[int] = [0, 1, 2], elev: float = 60, azim: float = -140
    ) -> None:
        """Plot 3d PCA"""
    ...
    def knn_imputation(
        ...
        if balanced:
            ...
            self.knn = knn_distance_matrix(
                space, metric=metric, k=k, mode="distance", n_jobs=n_jobs
            )
    ...
    def estimate_transition_prob(
        ...
        if knn_random:
            ...
            if cell_idx_use is None:
                ...
                self.embedding_knn = calculate_embedding_knn_with_cell_idx(
                    embedding_original=self.embedding,
    ...
    def prepare_markov(
        ...
        if direction == "forward":
            ...
            self.tr = np.array(
                (self.transition_prob[cells_ixs, :][:, cells_ixs]).T, order="C"
            )
        else:
    ...
    def run_markov(
        ...
        self.diffused = diffusor.diffuse(
            starting_p, self.tr, n_steps=n_steps, mode=mode
        )[0]
    ...
    def plot_mc_results_as_density(self, args={}):
        ...
        plt.scatter(
            self.embedding[self.ixs_mcmc, 0],
            self.embedding[self.ixs_mcmc, 1],
            c=diffused_n,
            **args_,
        )
        plt.axis("off")
    ...
    def plot_grid_arrows(
        ...
        **quiver_kwargs: Any,
        ...
        print(
            "This function is deprecated after celloracle ver 0.9.4. Please see our newest tutorial for the alternative functions. \nhttps://morris-lab.github.io/CellOracle.documentation/tutorials/index.html"
        )
...
def calculate_embedding_knn_with_cell_idx(
    embedding_original, cell_idx_use, n_neighbors, n_jobs=4
):
    """
