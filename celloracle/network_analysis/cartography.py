...
@np.vectorize
def __plot_annotations(x, y, label, x_shift=0.05, y_shift=0.05, args={}):
...
### scatter plots
def plot_cartography_color_scatter(
    data_, gois=None, args_dot={}, args_line={}, args_annot={}
):
    data = data_.copy()
    data.columns = ["z", "p"]
    data = add_label_cartography(data)

    _plot_base_cartography(z_min=0, z_max=data.z.max(), args_line=args_line)

    labels = list(data.label_cartography.unique())
    labels.sort()
    for i in labels:
        tmp_data = data[data.label_cartography == i]
        plt.scatter(tmp_data.p, tmp_data.z, label=i, **args_dot)
...
def plot_cartography_kde(
    data_, gois=None, scatter=False, kde=True, args_kde={}, args_line={}, args_annot={}
):
    """
