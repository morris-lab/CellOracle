...
def plot_scatter_with_anndata(adata, obsm_key, cluster_column_name, args={}):
    ...
    for cluster, color in colors.items():
        ...
        plt.scatter(
            embedding[idx, 0], embedding[idx, 1], c=color, label=cluster, **args
        )


def figures_for_trajectories301(
    self,
    save_folder,
    ...
    plt.axis("off")
    plt.savefig(
        os.path.join(save_folder, f"scatter_{cluster_column_name}_{cluster}.png"),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(save_folder, f"pseudotime_{cluster_column_name}_{cluster}.png"),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder, f"differentiation_{cluster_column_name}_{cluster}.png"
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"differentiation_with_cells_of_interest_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )


def figures_for_perturb_analysis_301(
    self,
    save_folder,
    scale_for_pseudotime=30,
    scale_for_simulated=30,
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"quiver_full_on_cellls_of_interest_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder, f"quiver_ful_on_cluster_{cluster_column_name}_{cluster}.png"
        ),
        transparent=True,
    )
    ...
    plt.axis("off")
    plt.savefig(
        os.path.join(save_folder, f"quiver_grid_{cluster_column_name}_{cluster}.png"),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"quiver_grid_on_cells_of_interest_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder, f"inner_product_score_{cluster_column_name}_{cluster}.png"
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"inner_product_score_and_perturb_quiver_grid_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"inner_product_score_distribution_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )
    ...
    plt.savefig(
        os.path.join(
            save_folder,
            f"inner_product_score_distribution_box_plot_{cluster_column_name}_{cluster}.png",
        ),
        transparent=True,
    )
