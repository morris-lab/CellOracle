...
def scatter_value_to_grid_value(
    embedding, grid, value, method="knn", n_knn=30, n_poly=3
):
    """
    ...
    if method == "poly":
        value_on_grid = _polynomial_regression_old_ver(
            x, y, x_new, y_new, value, n_degree=n_poly
        )
...
def _polynomial_regression_old_ver(x, y, x_new, y_new, value, n_degree=3):
    def __conv(
        x, y, n_degree=3
    ):  # Make polynomial data for polynomial ridge regression
        dic = {}
