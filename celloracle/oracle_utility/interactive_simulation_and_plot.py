...
class Oracle_extended(Oracle_data_strage, Oracle_development_module):
    ...
    def interactive_plot_quiver(self, quiver_scale=DEFAULT_PARAMETERS["quiver_scale"]):
        plt.figure(None, (6, 6))
        plt.scatter(
            self.oracle.embedding[:, 0],
            self.oracle.embedding[:, 1],
            c="0.8",
            alpha=0.2,
            s=38,
            edgecolor=(0, 0, 0, 1),
        ...
        quiver_kwargs = dict(
            ...
            width=0.0045,
            edgecolors="k",
            color=self.oracle.colorandum,
            alpha=1,
        )
        plt.quiver(
            self.oracle.embedding[:, 0],
            self.oracle.embedding[:, 1],
            self.oracle.delta_embedding[:, 0],
            self.oracle.delta_embedding[:, 1],
            scale=quiver_scale,
            **quiver_kwargs,
        )
    ...
    def interactive_plot_grid(
        ...
        if self.n_grid is None:
            self.oracle.calculate_grid_arrows(
                smooth=0.8, steps=(n_grid, n_grid), n_neighbors=200
            )
        else:
    ...
    def _plot_quiver_for_a_cluster(
        self, cluster_column_name, cluster, quiver_scale, plot_whole_cells=False
    ):
        ...
        plt.scatter(
            self.oracle.embedding[ix_choice, 0],
            self.oracle.embedding[ix_choice, 1],
            c="0.8",
            alpha=0.2,
            s=38,
            edgecolor=(0, 0, 0, 1),
        ...
        quiver_kwargs = dict(
            ...
            width=0.0045,
            edgecolors="k",
            color=self.oracle.colorandum[ix_choice],
            alpha=1,
        )
        plt.quiver(
            self.oracle.embedding[ix_choice, 0],
    ...
    def interactive_plot_quiver_for_a_cluster(
        self,
        cluster_column_name,
        cluster,
        ...
        plt.title(f"Knockout simulation: {self.gene}")
        plt.subplot(1, 2, 1)
        self._plot_quiver_for_a_cluster(
