...
def seurat_object_to_anndata(file_path_seurat_object, delete_tmp_file=True):
    ...
    for assay_name in assay_names:
        results[assay_name] = make_anndata_from_files(
            assay_name=assay_name, folder=folder
        )
...
def make_anndata_from_files(assay_name, folder="./tmp"):
    ...
    # Make anndata
    adata = sc.AnnData(
        mm.X, obs=meta_data.reindex(cell_ids), var=pd.DataFrame(index=gene_ids)
    )
    ...
    for i in adata.obs.columns:
        if all(
            adata.obs["active_ident"].values.astype("object")
...
def main():
    ...
    # print(sys.argv[:])
    adata_dictionary = seurat_object_to_anndata(
        file_path_seurat_object=sys.argv[1], delete_tmp_file=True
    )
    ...
    if len(adata_dictionary.keys()) >= 2:
        for assay_name, adata in adata_dictionary.items():
            adata.write(
                filename=output_file_name.replace(".h5ad", f"_{assay_name}.h5ad")
            )
    else:
        for assay_name, adata in adata_dictionary.items():
