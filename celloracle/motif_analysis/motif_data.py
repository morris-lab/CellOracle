...
def load_motifs(motifs_name, force_download=False):
    ...
    if motifs_name not in MOTIFS_LIST:
        raise ValueError(
            "The motifs name was not in the list. Available motifs: ", MOTIFS_LIST
        )

    # Load data from local directory if file exits.
    path = os.path.join(parent_path[0], "motif_data", motifs_name)
    path_factor_data = os.path.join(
        parent_path[0], "motif_data", motifs_name.replace(".pfm", ".motif2factors.txt")
    )
    if (
        (force_download == False)
        & os.path.isfile(path)
        & os.path.isfile(path_factor_data)
    ):
        pass
    else:
        # Download pfm file
        ...
        path_factor_data = os.path.join(
            CELLORACLE_DATA_DIR,
            "motif_data",
            motifs_name.replace(".pfm", ".motif2factors.txt"),
        )
        backup_url = os.path.join(
            WEB_PAR_DIR, "motif_data", motifs_name.replace(".pfm", ".motif2factors.txt")
        )
